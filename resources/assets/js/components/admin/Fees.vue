<template>
    <section>
        <article class="article-page container-768">
            ...
            <h2 class="article-title mb-5">Fees</h2>
            ...
            <br />
            <br />
            <h4 class="article-title"  v-sup="totalFee" data-suffix=" Total Fees"></h4>
            <br />
            <br />
            <div class="div-select right-radius container-440 mb-3">
                <select @change="filterChange" v-model="filter" class="custom-select custom-select-lg">
                    <option selected="selected" hidden="hidden" disabled="disabled" value="null">Filter fees by...</option>
                    <option value="date_desc">Date recent first</option>
                    <option value="date_asc">Date oldest first</option>
                    <option value="fee_desc">Fee highest first</option>
                    <option value="fee_asc">Fee lowest first</option>
            ...
            <div class="text-center" v-if="totalCount > tripList.length">
                <a @click.prevent="getMoreTrip" href="#" class="btn btn-darkblue btn-size-360">Show More Fees</a>
            </div>
            <br />
        </article>
    ...
    export default {
        ...
        methods: {
            getTripList() {
                ...
                this.$http.post(this.URL.base + '/admin-fee-list', param, {})
                .then(response => {
                    var responseData = response.body;
                    vm.totalCount = responseData.totalCount;
                    vm.totalFee = responseData.totalFee;
