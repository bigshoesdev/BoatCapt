...
class ReviewController extends Controller
{
    ...
    public function __construct(User $user, Review $review, Trip $trip, Profile $profile, OwnerPresenter $ownerPresenter, TripPresenter $tripPresenter, ReviewPresenter $reviewPresenter)
	{
		$this->user = $user;
		$this->review = $review;
        $this->trip = $trip;
    ...
    public function index() 
    {
        ...
        return view('pages.owner.reviews', compact('param', 'userInfo'));
    }
    ...
    public function detail($tripId) 
    {
        ...
        $trip = $this->trip->with(['profile', 'bid', 'bid.profile', 'bid.captainInfo'])
                        ->where('tripId', $tripId)
                        ->where('bid_id', '<>', null)
        ...
        $trip_id = $trip[0]['id'];
        $captain_id = $trip[0]['bid']['profile']['user_id'];
        ...
        return view('pages.owner.leave-review', compact('param', 'tripInfo', 'reviewInfo'));
    }
    ...
    public function leaveReview(Request $request) 
    {
        ...
        try {
            ...
            $reviews = $this->review->where('to_user_id', $captain_id)->get();
            $rating = 0;
            ...
            $this->profile->where('user_id', $captain_id)
                    ->update([
                        'rating'    => $rating
                    ]);
        ...
        return redirect()->back()->with('status', 'Captain review was successfully updated.');
    }
}